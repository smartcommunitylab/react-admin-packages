...
>(props: FieldProps<T, S, F>) {
    ...
    const inferLabel = () => {
        ...
        if (
            formContext &&
            'resource' in formContext &&
            'source' in formContext
        ) {
            return translateLabel({
                resource: formContext['resource'],
                source: formContext['source'] + '.' + name,
            }) as string;
        }
    ...
    if (wasPropertyKeyModified) {
        ...
        label = ADDITIONAL_PROPERTY_FLAG in schema ? name : inferLabel();
    }

    //force label as ui:title because we want to override default behavior
    if (fieldUiSchema) {
        fieldUiSchema['ui:title'] = label;
    }
    ...
    const errorsComponent =
        ...
        ((schema.anyOf || schema.oneOf) &&
            !schemaUtils.isSelect(schema)) ? undefined : (
            <FieldErrorTemplate
                errors={__errors}
